...
public sealed class DiscordService(IDiscordToken discordToken, IOptions<DiscordOptions> options) : IDiscord
{
    ...
    public async Task<Result<DiscordUser>> Authorize(string code, CancellationToken cancellationToken = default)
    {
        ...
        {
            Id = ulong.Parse(user.Id),
            Name = user.GlobalName,
            AvatarUrl = $"https://cdn.discordapp.com/avatars/{user.Id}/{user.Avatar}.png",
        };
    }

    public async Task<Result<DiscordUser>> GetUser(ulong userId, CancellationToken cancellationToken = default)
    {
        using HttpClient client = new();
        client.BaseAddress = _baseUri;
        ...
        return new DiscordUser
        {
            ...
            AvatarUrl = user.Avatar is not null 
                ? $"https://cdn.discordapp.com/avatars/{user.Id}/{user.Avatar}.png" 
                : null,
        };
    }

    public async Task<Result<List<DiscordUser>>> GetMembers(CancellationToken cancellationToken = default)
    {
        using HttpClient client = new();
        client.BaseAddress = _baseUri;
        client.DefaultRequestHeaders.Add("Authorization", $"Bot {discordToken.Token}");
        
        var response = await client.GetAsync($"guilds/{options.Value.GuildId}/members?limit=30", cancellationToken);
        if (!response.IsSuccessStatusCode) return new Error(
            response.StatusCode.ToString(), 
            await response.Content.ReadAsStringAsync(cancellationToken));
        
        var guildMembersJson = await response.Content.ReadAsStringAsync(cancellationToken);
        var members = JsonSerializer.Deserialize<List<GuildMembersResponse>>(guildMembersJson, new JsonSerializerOptions
        {
            PropertyNamingPolicy = JsonNamingPolicy.SnakeCaseLower
        });
        if (members is null) return new Error("JSON", "Could not deserialize user");
        
        return members.Select(m => new DiscordUser
        {
            Id = ulong.Parse(m.User.Id),
            Name = m.Nick ?? m.User.GlobalName ?? m.User.Username,
            AvatarUrl = (m.Avatar ?? m.User.Avatar) is not null 
                ? $"https://cdn.discordapp.com/avatars/{m.User.Id}/{m.Avatar ?? m.User.Avatar}.png" 
                : null,
        }).ToList();
    }
